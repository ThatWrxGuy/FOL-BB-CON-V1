...
import { PageContainer, Card, CardHeader, CardTitle, CardContent, Button, Badge, Grid, LoadingOverlay } from '../components'

const mockBrief = {
  generated: '2024-01-20T10:30:00Z',
  period: 'Weekly',
  summary: 'Your strategic position strengthened this week with key wins in Health and Finance domains.',
  highlights: [
...
function ExecutiveBrief() {
  ...
  return (
    ...
    >
      ...
      {/* Summary */}
      <Card className="mb-6">
        <CardHeader title="Strategic Summary" subtitle={`Generated ${new Date(brief.generated).toLocaleDateString()}`} />
        <CardContent>
          ...
          {brief.highlights.map((item, i) => (
            <div 
              ...
              className={`flex items-start gap-3 p-4 rounded-lg border ${
                item.type === 'win' ? 'border-success/30 bg-success/5' :
                item.type === 'insight' ? 'border-primary/30 bg-primary/5' :
            ...
            >
              ...
              <div className="flex-1">
                ...
                <Badge variant={item.impact === 'high' ? 'destructive' : item.impact === 'medium' ? 'warning' : 'secondary'} className="mt-1">
                  {item.impact} impact
                </Badge>
              </div>
            </div>
