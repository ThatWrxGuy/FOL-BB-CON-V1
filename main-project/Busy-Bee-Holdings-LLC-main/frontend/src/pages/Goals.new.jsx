...
import { 
  ...
  Button, Badge, Progress, Grid, Input, 
  LoadingOverlay, EmptyState, Tabs, TabsList, TabsTrigger, TabsContent
} from '../components'
...
// Mock data
const mockGoals = [
  { id: 1, title: 'Run 5K three times per week', progress: 75, status: 'active', category: 'Health', domain: 'health' },
  { id: 2, title: 'Save $10,000 emergency fund', progress: 45, status: 'active', category: 'Finance', domain: 'finance' },
  { id: 3, title: 'Morning meditation routine', progress: 90, status: 'active', category: 'Mindset', domain: 'mindset' },
  { id: 4, title: 'Complete professional certification', progress: 30, status: 'paused', category: 'Career', domain: 'career' },
  { id: 5, title: 'Read 24 books this year', progress: 60, status: 'active', category: 'Learning', domain: 'mindset' },
  { id: 6, title: 'Weekly family game night', progress: 100, status: 'completed', category: 'Relationships', domain: 'relationships' },
]
      ...
      {filteredGoals.length > 0 ? (
        <Grid cols={{ default: 1, md: 2, lg: 3 }} className="gap-4">
          {filteredGoals.map(goal => (
            <GoalCard key={goal.id} goal={goal} onClick={() => console.log('Goal clicked:', goal.id)} />
          ))}
        </Grid>
      ) : (
        <EmptyState 
          ...
          description={search ? "Try adjusting your search" : "Create your first goal to get started"}
          action={
            <Button>
              <FiPlus className="w-4 h-4 mr-2" />
              Create Goal
