...
import { PageContainer, Card, CardHeader, CardTitle, CardContent, Grid, LoadingOverlay } from '../components'
          ...
          <Card key={cube.id} hover className="cursor-pointer">
            <CardContent className="p-6 text-center">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${
                cube.status === 'active' ? 'bg-success/20 text-success' : 'bg-secondary text-foreground-muted'
              }`}>
                {cube.status}
              </span>
            </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {CUBES.map(cube => (
                <div key={cube.id} className="flex items-center justify-between p-3 rounded-lg border border-border">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {['Strategic analysis updated', 'New tactical recommendations', 'Financial metrics synced'].map((item, i) => (
